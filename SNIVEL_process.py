...
from SNIVEL_tools import azi_elev, klobuchar, ecef2lla, gpsweekdow, getklobucharvalues, niell, dxyz2dneu
import os
...
samprate=1 #sample rate...currently not used
...
with open(sitefile, 'rt') as g:
    ...
    for grow in rows:
        ...
        for i in range (int(tstart),int(tstop)+1):
            ...
            if (int((gtime[a1[0]]-gtime[a0[0]])) == samprate):
